...
async function getAllUsers(req, res) {
  ...
  try {
    if (req.user_data.role < 3) {
      ...
      await knex
        .raw(
          query + where_query + ` order by id desc LIMIT 10 offset ${offset}`
        )
        .then((response) => {
            ...
            .then(async (response) => {
              await HELPERS.sendMail(
                ...
                "verifyemail",
                {
                  ...
                  verifyLink:
                    HELPERS.react_url + `verifyemail?token=${new_password}`,
                },
                "Verify Your Account"
              );
            });
